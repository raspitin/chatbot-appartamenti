...
VERSIONE FIXED: Risolti bug datetime e connessione Ollama
"""
...
# 🚨 CORS CONFIGURATO per Villa Celi
CORS(app, 
     origins=[
         'https://www.villaceli.it',
         'https://villaceli.it', 
         'http://localhost:3000',  # Per sviluppo locale
         'http://127.0.0.1:3000'   # Per sviluppo locale
     ],
     allow_headers=['Content-Type', 'Authorization', 'X-WP-Nonce'],
...
class Config:
    DB_PATH = os.environ.get('DB_PATH', 'data/villa_celi_2025.db')
...
# ====================================
# UTILITY FUNCTIONS per DATE (FIXED)
# ====================================

def ensure_datetime(date_obj):
    """Converte date/datetime a datetime per comparazioni sicure"""
    if isinstance(date_obj, date) and not isinstance(date_obj, datetime):
        ...
        try:
            ...
            logger.error(f"Formato data non valido: {date_obj}")
            return datetime.now()
    else:
        logger.error(f"Tipo data non supportato: {type(date_obj)}")
        return datetime.now()

def ensure_date(date_obj):
    """Converte datetime/date a date per consistenza"""
...
# ====================================
# FUNZIONI CORE BUSINESS LOGIC (FIXED)
# ====================================

def query_appartamenti(month, year, apartment_name=None):
    """Trova settimane libere sabato-sabato - VERSIONE FIXED"""
    try:
        ...
        # Range del mese - CONVERTITI A datetime per comparazioni sicure
        start_month_datetime = datetime(year, month, 1)
        if month == 12:
            end_month_datetime = datetime(year, 12, 31)
        else:
            end_month_datetime = datetime(year, month + 1, 1) - timedelta(days=1)

        logger.info(f"Cercando disponibilità per {month}/{year} dal {start_month_datetime.date()} al {end_month_datetime.date()}")
        ...
        for apt_name_to_check in apartments_to_check:
            try:
                ...
                cursor = conn.execute(occupied_query, (
                    apt_name_to_check,
                    end_month_datetime.strftime('%Y-%m-%d'),
                    start_month_datetime.strftime('%Y-%m-%d')
                ))
                ...
                for occ in occupied_periods_raw:
                    try:
                        ci_str, co_str = occ
                        # FIXED: Assicura che siano datetime per comparazioni
                        occupied_periods_for_apt.append({
                            'check_in': ensure_datetime(ci_str),
                            'check_out': ensure_datetime(co_str)
                        })
                ...
                current_saturday = start_month_datetime
                while current_saturday.weekday() != 5:
                    current_saturday -= timedelta(days=1)

                while current_saturday <= end_month_datetime:
                    week_start = current_saturday
                    week_end = week_start + timedelta(days=7)

                    # FIXED: Tutte le comparazioni ora usano datetime
                    if not (week_end < start_month_datetime or week_start > end_month_datetime):
                        is_week_free_for_apt = True
                        
                        for occupied in occupied_periods_for_apt:
                            # FIXED: Comparazioni datetime sicure
        ...
        logger.error(f"Errore calcolo disponibilità: {e}", exc_info=True)
        return []
...
def generate_ollama_response(prompt):
    """Fallback Ollama OTTIMIZZATO con gestione errori migliorata"""
...
# ====================================
# FLASK ENDPOINTS CON CORS
# ====================================

logger.info("🚀 [DEBUG] Registrando endpoint Flask con CORS per Villa Celi...")

@app.route('/api/health', methods=['GET'])
def health():
    """Endpoint di health check con test Ollama"""
    logger.info("🏥 [DEBUG] Health endpoint chiamato")
    
    # Test Ollama
    ollama_status = "offline"
    try:
        test_response = requests.get(f"http://{Config.OLLAMA_URL.split('//')[1].split('/')[0]}/api/version", timeout=2)
    ...
    response_data = {
        "status": "ok",
        "service": "paguro_receptionist_villa_celi_fixed",
        "timestamp": datetime.now().isoformat(),
        "location": "Palinuro, Cilento",
        "database": "connected" if os.path.exists(Config.DB_PATH) else "missing",
        "ollama_status": ollama_status,
        "features": ["datetime_fixed", "post_processing", "forced_cta", "villa_celi_focus", "cors_enabled"],
        "cors_allowed_origins": ["https://www.villaceli.it", "https://villaceli.it"],
        "fixes_applied": ["datetime_comparisons", "ollama_connection_handling", "error_logging"]
    }
...
def chat():
    ...
    try:
        ...
        logger.info(f"📨 [DEBUG] Chat request: '{message}' [Session: {session_id}]")
        
        # Gestisci la query con post-processing
        response_data = handle_query(message, session_id)
        response_data['session_id'] = session_id
        response_data['timestamp'] = datetime.now().isoformat()
        
        logger.info(f"📤 [DEBUG] Risposta: tipo={response_data.get('type')}, lunghezza={len(response_data.get('message', ''))}")
...
def test_api():
    ...
    response_data = {
        "message": "API Paguro FIXED per Villa Celi Palinuro funzionante!",
        "location": "Palinuro, Cilento",
        "timestamp": datetime.now().isoformat(),
        "status": "ok",
        "optimizations": ["datetime_bug_fixed", "post_processing", "forced_cta", "villa_celi_focus", "cors_enabled", "ollama_error_handling"],
        "cors_info": "CORS configurato per villaceli.it"
    }
...
if __name__ == "__main__":
    logger.info("🚀 [DEBUG] === AVVIO PAGURO FIXED CON CORS - VILLA CELI PALINURO ===")
    logger.info(f"📂 Database: {Config.DB_PATH}")
    logger.info(f"🤖 Ollama: {Config.OLLAMA_URL}")
    logger.info(f"🎯 Model: {Config.OLLAMA_MODEL}")
    logger.info(f"🏖️ Località: Palinuro, Cilento")
    logger.info(f"✨ Features: Datetime Fixed, Post-processing, CTA forzati, Focus Villa Celi, CORS enabled")
    logger.info(f"🌐 CORS Origins: https://www.villaceli.it, https://villaceli.it")
    ...
    # Test Ollama con gestione errori migliorata
    try:
        test_response = generate_ollama_response("Test connessione Villa Celi")
        if test_response:
            logger.info("✅ Ollama OK - Connessione riuscita")
        else:
            logger.warning("⚠️ Ollama Warning - Connessione fallita ma continuo con fallback")
    except Exception as e:
        logger.warning(f"⚠️ Ollama non disponibile: {e} - Sistema continuerà con risposte predefinite")
    
    # Avvia server Flask
    port = int(os.environ.get('PORT', 5000))
    host = os.environ.get('HOST', '0.0.0.0')
    
    logger.info(f"🚀 [DEBUG] Paguro FIXED con CORS è pronto su http://{host}:{port}")
    ...
    logger.info("🌐 [DEBUG] CORS configurato per: https://www.villaceli.it")
    logger.info("🔧 [DEBUG] Bug fixes: datetime comparisons, Ollama error handling")
